"""Tests for review queue Excel read/write."""
...
def test_review_queue_rows_round_trip(tmp_path: Path):
    ...
    path = tmp_path / "review_queue.xlsx"
    write_review_queue_rows(path, rows)
    loaded = read_review_queue(path)
    assert loaded == rows
...
def test_write_review_queue_rows_raises_when_locked(tmp_path: Path):
    ...
    with patch("dataroom.export.review_queue.write_table_xlsx_atomic", side_effect=_always_denied):
        with pytest.raises(ReviewQueueWriteError, match="Close review_queue.xlsx"):
